...
Lembre-se de salvar seu Modelo de Dados de Entidade depois de fazer uma modificação clicando no botão Salvar (o ícone do disquete). Nos bastidores, o Entity Designer gera um conjunto de classes Visual Basic .NET. Você pode visualizar essas classes abrindo o arquivo MoviesDBModel.Designer.vb a partir da janela Solution Explorer.

Não modifique o código no arquivo Designer.vb, pois suas alterações serão substituídas na próxima vez que você usar o Entity Designer. Se você quiser estender a funcionalidade das classes de entidade definidas no arquivo Designer.vb, então você pode criar *classes parciais* em arquivos separados.
...
O \_campo db é usado dentro da ação Index() para recuperar os registros da tabela de banco de dados Filmes. A \_expressão db. MovieSet representa todos os registros da tabela de banco de dados Filmes. O método ToList() é usado para converter o conjunto de filmes em uma coleção genérica de objetos de filme: Lista (de filme).
...
A exibição na Listagem 2 contém um loop **para cada** loop que itera através de cada registro de filme e exibe os valores das propriedades título e diretor do registro do filme. Observe que um link Editar e Excluir é exibido ao lado de cada registro. Além disso, um link Adicionar filme é exibido na parte inferior da exibição (ver Figura 7).

**Figura 7 – A visão do Índice**

![clip_image014](creating-model-classes-with-the-entity-framework-vb/_static/image7.jpg)

A exibição Índice é uma *exibição digitada*. A exibição &lt;Índice tem&gt; uma diretiva %@ Page % que inclui um atributo Herda. O atributo Herdas lança a propriedade ViewData.Model para uma coleção de lista genérica fortemente digitada de objetos de filme – uma lista(de filme).
...
**Listagem 3 – Controladores\HomeController.vb (Adicionar métodos)**
...
**Listagem 4 – Controladores\HomeController.vb (Métodos de edição)**
...
**Listagem 6 -- \Controllers\HomeController.vb (Exclusão de ação)**
