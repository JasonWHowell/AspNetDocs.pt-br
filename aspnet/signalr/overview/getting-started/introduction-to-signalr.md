...
O signalr é uma abstração de alguns dos transportes que são necessários para realizar o trabalho em tempo real entre o cliente e o servidor. O signalr primeiro tenta estabelecer uma conexão WebSocket, se possível. WebSocket é o transporte ideal para Signalr porque ele tem:

* O uso mais eficiente da memória do servidor.
* A menor latência.
* Os recursos mais subjacentes, como a comunicação full duplex entre o cliente e o servidor.
* Os requisitos mais estritos, WebSocket requer o servidor:
...
2. Se JSONP estiver configurado (ou seja, o `jsonp` parâmetro será definido como `true` quando a conexão for iniciada), a sondagem longa será usada.
...
Neste exemplo, o nome do hub é identificado com o `H` parâmetro; o nome do método é identificado com o `M` parâmetro e os dados que estão sendo enviados para o método são identificados com o `A` parâmetro. O aplicativo que gerou essa mensagem é criado no tutorial em [tempo real de alta frequência](tutorial-high-frequency-realtime-with-signalr.md) .
